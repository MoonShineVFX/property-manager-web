  ...
  return <div className='grid grid-row-5 auto-rows-fr w-full h-full'>
    ...
    <div className='relative row-span-2 w-full h-full flex justify-center items-center z-10'>
      <FetchStateSwitcher
        ...
        resultElement={
          <div
            className='flex flex-col gap-8 w-full items-center mx-4 py-8 px-8 bg-gray-700 max-w-xs rounded-xl drop-shadow-xl'>
            {membersData &&
              <DropdownMenu selectValue={selectedTeam} onChange={data => dispatch(setSelectedTeam(data))}
                            dataList={membersData}/>}
            {selectedTeam &&
              <DropdownMenu selectValue={selectedMember} onChange={data => dispatch(setSelectedMember(data))}
                            dataList={selectedTeam.value}/>}
          </div>
        }
      />
    </div>
    ...
    <div className='relative row-span-3 w-full h-full flex flex-col items-center justify-center'>
      <FetchStateSwitcher
        ...
        welcomeMessage={['請在上方選擇同事', '或者直接掃描 QR Code', '接著輸入產編轉移']}
      />
